...
(defun ascii-range (c0 c1 string &key (start 0))
  "match an ASCII character in the range (<= c0 (char-code c) c1)"
  (when (and (< start (length string))
             (<= c0 (char-code (char string start)) c1))
    (values (1+ start) (string (char string start)))))
...
(defun m-to-n (f m n string &key (start 0))
  "match f, repeated at least m and up to n times"
  )

(defun m-to-n-only (f m n string &key (start 0))
  "match f, repeated at least m and at most n times"
  )

(defun n+ (f n string &key (start 0))
  "match f, repeated n or more times"
  )

(defun * (f string &key (start 0))
  "match f 0 or more times"
  (n+ f 0 string :start start))
